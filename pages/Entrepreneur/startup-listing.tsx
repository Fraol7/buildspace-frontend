...
export default function StartupListing() {
  ...
  const totalPages = Math.ceil(DUMMY_STARTUPS.length / ITEMS_PER_PAGE)
  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE
  const endIndex = startIndex + ITEMS_PER_PAGE
  const currentStartups = DUMMY_STARTUPS.slice(startIndex, endIndex)
      ...
      <div className="container mx-auto px-6 py-12">
        ...
        <div className="text-center">
          <div className="inline-flex items-center gap-2 bg-white/90 backdrop-blur-sm text-sky-700 font-semibold rounded-full py-3 px-6 shadow-lg border border-sky-200/50">
            ...
            Showing {startIndex + 1}-{Math.min(endIndex, DUMMY_STARTUPS.length)} of {DUMMY_STARTUPS.length} startups
          </div>
        </div>
      </div>
    </div>
